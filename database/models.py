...
class Restaurant(Base):
    ...
    my_service_rating = Column(
        Float, CheckConstraint("my_service_rating >= 1 AND my_service_rating <= 10")
    )
    my_food_rating = Column(
        Float, CheckConstraint("my_food_rating >= 1 AND my_food_rating <= 10")
    )
    my_coffee_rating = Column(
        Float, CheckConstraint("my_coffee_rating >= 1 AND my_coffee_rating <= 10")
    )
    my_interior_rating = Column(
        Float, CheckConstraint("my_interior_rating >= 1 AND my_interior_rating <= 10")
    )
    ...
    yandex_url_status = Column(
        String(20), default="unknown"
    )  # ok | broken | unreachable | not_found | unknown
...
class Review(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "restaurant_id", "yandex_review_id", name="unique_review_per_restaurant"
        ),
    )

    def __repr__(self):
        return (
            f"<Review(id={self.id}, restaurant_id={self.restaurant_id}, "
