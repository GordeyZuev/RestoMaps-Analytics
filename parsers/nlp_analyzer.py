...
class ReviewProcessor:
    def __init__(self):
        self.positive_words = {
            'вкусно', 'вкусный', 'вкусная', 'вкусные', 'вкусное', 'отлично', 'прекрасно',
            'восхитительно', 'замечательно', 'рекомендую', 'советую', 'супер', 'класс',
            'шикарно', 'свежий', 'свежая', 'свежее', 'свежие', 'качественный', 'вежливый',
            'внимательный', 'профессиональный', 'уютно', 'комфортно', 'чисто', 'быстро', 'приятно',
            'великолепное', 'великолепный', 'великолепная', 'великолепные', 'невероятная', 'невероятный'
        }

        self.negative_words = {
            'плохо', 'ужасно', 'отвратительно', 'кошмар', 'не советую', 'разочарован',
            'разочарована', 'долго', 'грязно', 'грубый', 'дорого', 'невкусно', 'испорченный',
            'холодный', 'горький', 'пересоленный', 'медленно', 'медленный'
        }

        self.negation_words = {
            'не', 'нет', 'ни', 'без', 'никак', 'нисколько', 'отсутствует',
            'невозможно', 'никакой', 'ничего'
        }

        self.intensifiers = {
            'очень', 'крайне', 'совсем', 'абсолютно', 'полностью', 'совершенно',
            'невероятно', 'необычайно', 'чрезвычайно', 'сильно'
        }

        self.diminishers = {
            'слегка', 'немного', 'чуть', 'чуть-чуть', 'довольно', 'вроде'
        }
    ...
    def analyze_sentiment_with_negation(self, text):
        ...
        while i < len(words):
            ...
            if word in self.positive_words:
                score = 1.5
                sentiment_score += score * intensifier_strength * (-1 if negation_active else 1)
                negation_active = False
                intensifier_strength = 1.0
    ...
    def calculate_sentiment_score(self, text):
        ...
        strong_positive = any(phrase in text_lower for phrase in
                              ['обязательно вернусь', 'рекомендую всем', 'превосходно', 'восхитительно'])
        strong_negative = any(phrase in text_lower for phrase in
                              ['никому не рекомендую', 'ужасное', 'кошмар', 'отвратительно', 'больше не пойду'])
            ...
            if indicator in text_lower:
                if tag in ['Вкусная еда', 'Свежие продукты', 'Качественные продукты']:
                    ...
                    for negation_word in ['не', 'ни']:
                        if (f'{negation_word} {indicator}' in text_lower or
                            f'{negation_word}{indicator}' in text_lower or
                            f'{negation_word} очень {indicator}' in text_lower or
                            f'{negation_word} слишком {indicator}' in text_lower or
                                f'{negation_word} совсем {indicator}' in text_lower):
                            negation_found = True
                            break
    ...
    def _analyze_service_quality(self, text_lower, tags):
        ...
        if 'быстро' in text_lower and any(word in text_lower for word in ['обслуживание', 'подача', 'принесли']):
            if 'не быстро' not in text_lower:
    ...
    def _analyze_prices_value(self, text_lower, tags):
        ...
        if 'цены' in text_lower:
            ...
            elif any(word in text_lower for word in ['доступн', 'демократичн', 'приемлем']):
