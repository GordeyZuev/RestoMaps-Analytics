...
def render_weekly_ratings_chart(reviews_df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    valid_reviews = reviews_df[
        (reviews_df['original_date'].notna()) &
    ...
    valid_reviews['date'] = pd.to_datetime(valid_reviews['original_date'], errors='coerce')
    ...
    weekly_stats = valid_reviews.groupby('year_week').agg({
        'rating': ['mean', 'count'],
        'date': ['min', 'max']
    }).round(2)
...
def render_weekly_chart(weekly_data: pd.DataFrame) -> None:
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='lines+markers',
        name='Средняя оценка',
        line=dict(color='#243071', width=3),
        marker=dict(size=8, color='#243071'),
        hovertemplate='<b>%{x}</b><br>Средняя оценка: %{y:.2f}<br>Количество отзывов: %{customdata}<br><extra></extra>',
    ...
    fig.update_layout(
        ...
        font=dict(size=12, color='#243071'),
        title_font=dict(size=14, color='#243071'),
...
def render_analytics_charts(filtered_df: pd.DataFrame) -> None:
    ...
    with col3:
        ...
        fig_types = px.pie(
            ...
            title="Типы мест",
            color_discrete_sequence=['#243071', '#4A90E2', '#7BB3F0', '#A8D0F0', '#C8E6F5']
        )
