...
def render_rating_metric(title: str, rating: float | None, scale: str = "/10") -> None:
...
def render_summary_metrics(
    restaurants_df: pd.DataFrame, filtered_df: pd.DataFrame
) -> None:
    ...
    with col2:
        visited_count = (
            int(restaurants_df[restaurants_df["visited"]].shape[0])
...
def render_restaurant_info(_restaurant_row: pd.Series, reviews_df: pd.DataFrame) -> None:
